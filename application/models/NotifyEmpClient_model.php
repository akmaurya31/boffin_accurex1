...
class NotifyEmpClient_model extends CI_Model {
    ...
    public function count_filtered_jobs333($code = '', $name = '') {
     ...
     public function extra_get_filtered_jobs($limit, $offset, $filters, &$total = 0) {
        ...
        $this->db->from('emp_job_notifications');

        // echo $filters['status']; die("ASdf");
        if($filters['tabType']=='Client'){
            $this->db->where('client_id >', 0);
            $this->db->where('show_emp_id', $sessionData->user_ID);
        ...
        if (!empty($filters['search_code'])) {
            ...
            $this->db->or_like('n_status', $filters['search_code']);
            $this->db->group_end();  
        }
